...
  }
    ...
    onSubmit(form: NgForm) {
      ...
      if (form.value.receiverType === 'Private') {
        this.firestore.collection('Log').add({
          adminId: this.Log.uid,
          adminName: this.Log.displayName,
          action : 'Message Sent' ,
          log: 'New message has been sent to the user with the email address ' + form.value.receiver, time : this.date 
        });
        ...
        this.firestore.collection('Log').add({
          ...
          adminName: this.Log.displayName,
          action : 'Message Sent' ,
          log: 'Broadcast message has been sent Globally',
          time : this.date
        });
        ...
        this.firestore.collection('Log').add({
          ...
          adminName: this.Log.displayName,
          action : 'Message Sent' ,
          log: 'Broadcast message has been sent to all Professionals',
          time : this.date });
      } else if (form.value.receiverType === 'User') {
        this.firestore.collection('Log').add({adminId: this.Log.uid,
          adminName: this.Log.displayName,
          action : 'Message Sent' ,
          log: 'Broadcast message has been sent to all Users',
          time : this.date });
      }
  ...
  onDelete(id: string) {
    if (confirm('Are you sure, you want to delete this message?')) {
      ...
      this.firestore.collection('Log').add({
        ...
        log: 'A sent message has been deleted',
        time : this.date
      });
  ...
  RecOnDelete(id: string) {
    if (confirm('Are you sure, you want to delete this message?')) {
      ...
      this.firestore.collection('Log').add({
        ...
        adminName: this.Log.displayName,
        action : 'Message Deleted' ,
        log: 'A received message has been deleted',
        time : this.date
      });
      this.toastr.success('Message deleted sucessfully', 'Jamboree.MessageDelete');
    }
